...
<template>
  <div class="py-12">
    ...
    >
      ...
      >
        ...
        >
          ...
          <td class="px-6 py-4 text-dark whitespace-nowrap">{{ concrete.class }}</td>
          <td class="px-6 py-4 text-dark whitespace-nowrap">{{ concrete.strength }}</td>
    ...
    <div v-else-if="activeViewMode === 'slider'" class="container mx-auto">
      <div
          class="bg-background relative overflow-hidden rounded-xl p-6 pb-12 shadow-lg"
      >
        <div class="flex">
        ...
        <button
            @click="prevSlide"
            class="bg-background absolute top-1/2 left-4 z-10 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full text-xl shadow-md transition-all hover:bg-gray-100"
        >
          ❮
        </button>
        <button
            @click="nextSlide"
            class="bg-background absolute top-1/2 right-4 z-10 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full text-xl shadow-md transition-all hover:bg-gray-100"
        >
          ❯
        </button>
    ...
    <div
        v-if="showModal && selectedConcrete"
        class="bg-opacity-50 fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"
        @click.self="closeModal"
    >
      <div
          class="bg-background max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-xl p-6 shadow-2xl"
      >
        <div class="mb-4 flex items-center justify-between">
          <h2
              class="text-2xl font-bold"
              :style="`color: ${getCategoryColor(selectedConcrete.category)};`"
          >
            {{ selectedConcrete.mark }} ({{ selectedConcrete.class }})
          </h2>
          <button
              @click="closeModal"
        ...
        <div class="mb-6 grid grid-cols-1 gap-6 md:grid-cols-2">
          <div>
            <img
                :src="selectedConcrete.image"
                :alt="selectedConcrete.mark"
                class="h-auto w-full rounded-lg object-cover"
            />
          </div>
          <div>
            <h3 class="mb-4 text-xl font-semibold">
              {{ selectedConcrete.description }}
            </h3>
            ...
            <div class="mb-4 rounded-lg bg-gray-50 p-4">
              ...
              <ul class="space-y-2">
                <li class="flex justify-between">
                  <span class="text-gray-600">Марка бетону:</span>
                  <span class="font-medium text-gray-800">{{
                      selectedConcrete.mark
                    }}</span>
                </li>
                <li class="flex justify-between">
                  <span class="text-gray-600">Клас міцності:</span>
                  <span class="font-medium text-gray-800">{{
                      selectedConcrete.class
                    }}</span>
                </li>
                <li class="flex justify-between">
                  <span class="text-gray-600">Міцність на стиск:</span>
                  <span class="font-medium text-gray-800">{{
                      selectedConcrete.strength
                    }}</span>
                </li>
                <li class="flex justify-between">
                  <span class="text-gray-600">Категорія:</span>
                  <span
                      class="rounded-full px-3 py-0.5 text-sm font-medium text-white"
                      :style="`background-color: ${getCategoryColor(selectedConcrete.category)};`"
                  >
                    {{
                      filterCategories.find(
                          (c) => c.id === selectedConcrete.category,
                      )?.name || selectedConcrete.category
                    }}
                  </span>
                </li>
              </ul>
            ...
            <div>
              ...
              <ul class="space-y-2">
                <li
                    v-for="app in selectedConcrete.applications"
                    :key="app"
                    class="flex items-start"
                >
                  <span
        ...
        <div class="mt-6 border-t border-gray-200 pt-6">
          <h4 class="mb-3 text-lg font-medium">Додаткова інформація:</h4>
          <p class="text-gray-600">
            {{ selectedConcrete.mark }} - це
            {{ selectedConcrete.description.toLowerCase() }}. Бетон цієї марки
            характеризується міцністю {{ selectedConcrete.strength }} та
            відповідає класу {{ selectedConcrete.class }}. Має оптимальне
            співвідношення якості та ціни для вказаних областей застосування.
          </p>

          <div class="mt-6 flex justify-end space-x-4">
            <button
                @click="closeModal"
                class="rounded-md border border-gray-300 px-5 py-2.5 font-medium text-gray-700 transition-colors hover:bg-gray-50"
            >
              Закрити
            </button>
            <button
                class="rounded-md px-5 py-2.5 font-medium text-white transition-colors hover:opacity-90"
                :style="`background-color: ${getCategoryColor(selectedConcrete.category)};`"
            >
              Замовити
            </button>
